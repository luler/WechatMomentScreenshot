...
var avatarURL = [];
xhrGet('avatarURL.json', function (result) {
    avatarURL = JSON.parse(result)
});
var emoticon = [];
...
document.getElementById('avatar').style.backgroundImage = 'url(' + (localStorage.getItem('avatar') || 'image/common/HTB1yE4fMmzqK1RjSZFp761kSXXal.png') + ')';
...
function getArticleInfo() {
    ...
    mdui.prompt('公众号文章链接', function (value) {
        var xhr = new XMLHttpRequest;
        xhr.open('GET', 'get_article_info.php?url=' + encodeURIComponent(value)); //获取标题和封面的服务器
...
document.getElementById('generate').addEventListener('click', function () {
    ...
    html2canvas(document.getElementById('fakeWechatMoment'), {
        ...
        mdui.alert(''
            ...
            + '<p>' + error + '</p>'
            + '<pre>' + error.stack + '</pre>'
            + '<p>你可以通过 <a href="https://github.com/luler/WechatMomentScreenshot/issues" target="_blank">Issue</a> 向作者反馈 BUG～</p>'
            + '</div>',
            '生成失败'
        );
    }).finally(function () {
